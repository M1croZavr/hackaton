...
def main(args):
    ...
    if args.mode == 'train':
        assert len(subdirs(args.train_source_img_dir)) == 1
        assert len(subdirs(args.train_reference_img_dir)) == args.num_domains
        loaders = Munch(src=get_train_loader(root=args.train_source_img_dir,
                                             which='source',
                                             img_size=args.img_size,
                                             batch_size=args.batch_size,
                                             num_workers=args.num_workers),
                        ref=get_train_loader(root=args.train_reference_img_dir,
                                             which='reference',
                                             img_size=args.img_size,
                                             batch_size=args.batch_size,
                                             num_workers=args.num_workers))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=1e-4,
                        help='Learning rate for the discriminator and the generator')
